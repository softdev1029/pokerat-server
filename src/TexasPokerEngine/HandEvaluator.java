...
public class HandEvaluator {
	...
	public List<Card> getBestCards()
	{
		...
		if(type == HandValueType.ROYAL_FLUSH) {
			for(int i = 0; i < 5; i ++)
				cardList.add(new Card(Card.ACE - i, flushSuit));
		}
		else if(type == HandValueType.STRAIGHT_FLUSH) {
			for(int i = 0; i < 5; i ++)
				cardList.add(new Card(rankings[1] - i, flushSuit));
		}
		else if(type == HandValueType.FOUR_OF_A_KIND) {
			for(int i = 0; i < 4; i ++)
				cardList.add(new Card(rankings[1], i));
		}
		else if(type == HandValueType.FULL_HOUSE) {
			for(Card card : cards) {
				if(card.getRank() == rankings[1])
					cardList.add(card);
			}
			for(Card card : cards) {
				if(card.getRank() == rankings[2])
					cardList.add(card);
				if(cardList.size() == 5)
					break;
			}
		}
		else if(type == HandValueType.FLUSH) {
			for(int i = 1; i <= 5; i ++)
				cardList.add(new Card(rankings[i], flushSuit));
		}
		else if(type == HandValueType.STRAIGHT) {
			int rank = rankings[1];
			for(Card card : cards) {
		...
		else if(type == HandValueType.THREE_OF_A_KIND) {
			for(Card card : cards) {
				if(card.getRank() == rankings[1])
					cardList.add(card);
			}
		}
		else if(type == HandValueType.TWO_PAIRS) {
			for(Card card : cards) {
				if(card.getRank() == rankings[1])
					cardList.add(card);
			}
		...
		else if(type == HandValueType.ONE_PAIR) {
			for(Card card : cards) {
				if(card.getRank() == rankings[1])
					cardList.add(card);
			}
	...
	public List<Card> getBestWholeCards()
	{
		List<Card> cardList = getBestCards();
		for(int i = 0; i < cards.length; i ++) {
			for(int j = i + 1; j < cards.length; j ++) {
				if(cards[i].getRank() < cards[i].getRank()) {
					Card tmp = cards[i];
					cards[i] = cards[j];
					cards[j] = tmp;
				}
		...
		for(int i = 0; i < cards.length; i ++) {
			...
			for(Card card : cardList) {
				if(card.getRank() == cards[i].getRank() && card.getSuit() == cards[i].getSuit())
					flag = false;
			}
			if(flag)
			{
